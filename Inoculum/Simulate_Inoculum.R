...
optim.fit.fun = function(i,model,data.A,data.N,xc.select){
  ...
  mat.piecewise = function(x,t,lambdaA,k,c,x.c,mu,sigma,P0) { 
    P<-if (((x>=t*lambdaA) & (x<x.c))) {P0*dnorm(x-t*lambdaA,mean=mu,sd=sigma)} else if (((x>=t*lambdaA) & (x>=x.c) & ((x-t*lambdaA)<=x.c))) {
      P0*dnorm(x-t*lambdaA,mean=mu,sd=sigma)*exp(-(c/lambdaA)*(x-x.c))} else if (((x>=t*lambdaA) & (x>=x.c) & ((x-t*lambdaA)>x.c))) {
        P0*dnorm(x-t*lambdaA,mean=mu,sd=sigma)*exp(-c*t)} else if (((x<t*lambdaA) & (x<x.c) & ((t*lambdaA-x)>=(1-x.c)))) {
          k*P0*dnorm(1+x-t*lambdaA,mean=mu,sd=sigma)*exp(-(1*c/lambdaA)*(1-x.c))} else if (((x<t*lambdaA) & (x<x.c) & ((t*lambdaA-x)<(1-x.c)))) {
            k*P0*dnorm(1+x-t*lambdaA,mean=mu,sd=sigma)*exp(-(c/lambdaA)*(t*lambdaA-x-(1-1)*x.c))} else if (((x<t*lambdaA) & (x>=x.c) & ((t*lambdaA-x)>=(1-x.c)))) {
              k*P0*dnorm(1+x-t*lambdaA,mean=mu,sd=sigma)*exp(-(1*c/lambdaA)*(1-x.c))*exp(-(c/lambdaA)*(x-x.c))} else if (((x<t*lambdaA) & (x>=x.c) & ((t*lambdaA-x)<(1-x.c)))) {
                k*P0*dnorm(1+x-t*lambdaA,mean=mu,sd=sigma)*exp(-(c/lambdaA)*(t*lambdaA-1*x.c))}
    ...
    tmp = try({
      ...
      Ringfun=function(t){
        integrand=Vectorize(function(x){
          mat.piecewise(x,t=t,lambdaA=paravec[1],k=paravec[2],c=paravec[3],x.c=xc,mu=paravec[4],sigma=paravec[5],P0=paravec[6])
        })
...
for (i in 1:55){
  ...
  write.csv(optim.joint,paste("Inoculum_",model.select,".csv",sep=""),row.names=FALSE)
}
