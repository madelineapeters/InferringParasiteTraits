model.select = "full" #"full" or "reduced" parasite maturation model form
set = 1 #combination of epsilon and frac % retics being considered
c = 1 #index of variable that is being varied

#Specify varied parameter (col 1 = epsilon, col 2 = frac retics)
epsilon.frac.opt = matrix(c(1,0.5,0.5,1,0.25,0.8),nrow=3,byrow=T)
epsilon = epsilon.frac.opt[set,1]
frac = epsilon.frac.opt[set,2]
...
optim.fit.fun = function(i,model,data.A,data.N,xc.select){
  ...
  mat.piecewise = function(x,t,l,Beta,c,x.c,mu,sigma,P0) { 
    P<-if (((x>=t*l) & (x<x.c))) {P0*dnorm(x-t*l,mean=mu,sd=sigma)} else if (((x>=t*l) & (x>=x.c) & ((x-t*l)<=x.c))) {
      P0*dnorm(x-t*l,mean=mu,sd=sigma)*exp(-(c/l)*(x-x.c))} else if (((x>=t*l) & (x>=x.c) & ((x-t*l)>x.c))) {
        P0*dnorm(x-t*l,mean=mu,sd=sigma)*exp(-c*t)} else if (((x<t*l) & (x<x.c) & ((t*l-x)>=(1-x.c)))) {
          Beta*P0*dnorm(1+x-t*l,mean=mu,sd=sigma)*exp(-(1*c/l)*(1-x.c))} else if (((x<t*l) & (x<x.c) & ((t*l-x)<(1-x.c)))) {
            Beta*P0*dnorm(1+x-t*l,mean=mu,sd=sigma)*exp(-(c/l)*(t*l-x-(1-1)*x.c))} else if (((x<t*l) & (x>=x.c) & ((t*l-x)>=(1-x.c)))) {
              Beta*P0*dnorm(1+x-t*l,mean=mu,sd=sigma)*exp(-(1*c/l)*(1-x.c))*exp(-(c/l)*(x-x.c))} else if (((x<t*l) & (x>=x.c) & ((t*l-x)<(1-x.c)))) {
                Beta*P0*dnorm(1+x-t*l,mean=mu,sd=sigma)*exp(-(c/l)*(t*l-1*x.c))}
    ...
    tmp = try({
      ...
      Ringfun=function(t){
        integrand=Vectorize(function(x){
          mat.piecewise(x,t=t,l=paravec[1],Beta=paravec[2],c=paravec[3],x.c=xc,mu=paravec[4],sigma=paravec[5],P0=paravec[6])
        })
...
para.list$pR = pRD
para.list$rho = rho
para.list$epsilon = epsilon
para.list$phi = 0
para.list$frac = frac
para.list$bias = "stages"
para.list$xc = 0.5

lower = c(kappaN,kappaA,gammaN,0,sigma1,sigma2,pR,rho,epsilon,phi)[c]
upper = c(1,1,3,0.9,2,sigma2,0.03,300,epsilon,3)[c]

para.list[,c] = seq(lower,upper,length.out=50)

optim.joint = data.frame()
starting.i = 1

for (i in starting.i:nrow(para.list)){
  ...
  #Specify RBC age structure of initial cohort
  frac = as.double(para.list$frac[i])
  
  #Fraction misclassified
  epsilon = as.double(para.list$epsilon[i])
  
  #Function to create simulated data set with specified measurement bias applied; returns "Time", "Ring" and "Late" columns.
  DDE.opt = function(parameters,bias,frac){
    ...
    DDE.fun = function (t, y, parms){
      ...
      alpha = parms[6] #shape 1 term for Beta distribution of parasite age
      beta = parms[7] #shape 2 term
      rho = parms[8] #reticulocyte preference
      ...
      if (is.infinite(dbeta(1,alpha,beta))){
        #Transfused population differential equations
        dPRdt = - pulsebeta(P0,alpha,beta,0.5-t)
        dPTdt = pulsebeta(P0,alpha,beta,0.5-t) - gamma*PT - pulsebeta(P0,alpha,beta,0.75-t)*ST
        dPSdt = pulsebeta(P0,alpha,beta,0.75-t)*ST - gamma*PS - phi*PS - pulsebeta(P0,alpha,beta,0.9999-t)*SM
        dPSQdt = phi*PS - pulsebeta(P0,alpha,beta,0.9999-t)*SQ
        
        #Uninfected population differential equations
        dUNdt = - kappa*UN/(UN+rho*UR)*pulsebeta(P0,alpha,beta,0.9999-t)*(SQ+SM) - kappa*UN/(UN+rho*UR)*pulsebeta(P0H,1,1,0.9999-t)
        dURdt = - kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0,alpha,beta,0.9999-t)*(SQ+SM) - kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0H,1,1,0.9999-t)
        
        #Newly infected population differential equations
        dRNdt = kappa*UN/(UN+rho*UR)*pulsebeta(P0,alpha,beta,0.9999-t)*(SQ+SM)
        dRRdt = kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0,alpha,beta,0.9999-t)*(SQ+SM) 
        dLTNdt = 0
        dLTRdt = 0
        dLSNdt = 0
        dLSRdt = 0
        ...
        dHRdt = kappa*UN/(UN+rho*UR)*pulsebeta(P0H,1,1,0.9999-t) + kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0H,1,1,0.9999-t)
        dHLTdt = 0
        dHLSdt = 0
        dHSQdt = 0
      } else {
        ...
        dPRdt = - pulsebeta(P0,alpha,beta,0.5-t)
        dPTdt = pulsebeta(P0,alpha,beta,0.5-t) - gamma*PT - pulsebeta(P0,alpha,beta,0.75-t)*ST
        dPSdt = pulsebeta(P0,alpha,beta,0.75-t)*ST - gamma*PS - phi*PS - pulsebeta(P0,alpha,beta,1-t)*SM
        dPSQdt = phi*PS - pulsebeta(P0,alpha,beta,1-t)*SQ
        
        #Uninfected population differential equations
        dUNdt = - kappa*UN/(UN+rho*UR)*pulsebeta(P0,alpha,beta,1-t)*(SQ+SM) - kappa*UN/(UN+rho*UR)*pulsebeta(P0H,1,1,1-t)
        dURdt = - kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0,alpha,beta,1-t)*(SQ+SM) - kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0H,1,1,1-t)
        
        #Newly infected population differential equations
        dRNdt = kappa*UN/(UN+rho*UR)*pulsebeta(P0,alpha,beta,1-t)*(SQ+SM)
        dRRdt = kappa*rho*UR/(UN+rho*UR)*pulsebeta(P0,alpha,beta,1-t)*(SQ+SM) 
        dLTNdt = 0
        dLTRdt = 0
        dLSNdt = 0
        dLSRdt = 0
      ...
      if ((t>0.5)&(t<=0.75)){
        ...
        dRNdt = dRNdt - kappa*lag0.5[1]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.5)))+exp(-phi*(t-0.5))*exp(-gamma*(t-0.5))) 
        
        dRRdt = dRRdt - kappa*rho*lag0.5[2]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.5)))+exp(-phi*(t-0.5))*exp(-gamma*(t-0.5)))
        
        dLTNdt = kappa*lag0.5[1]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.5)))+exp(-phi*(t-0.5))*exp(-gamma*(t-0.5))) - gamma*LTN
        
        dLTRdt = kappa*rho*lag0.5[2]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.5)))+exp(-phi*(t-0.5))*exp(-gamma*(t-0.5))) - gamma*LTR
      ...
      if (t>0.75){
        ...
        dRNdt = dRNdt - kappa*lag0.5[1]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*0.25))+exp(-phi*0.25)*exp(-gamma*0.25))*exp(-gamma*(t-0.75))
        
        dRRdt = dRRdt - kappa*rho*lag0.5[2]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*0.25))+exp(-phi*0.25)*exp(-gamma*0.25))*exp(-gamma*(t-0.75))
        
        dLTNdt = kappa*lag0.5[1]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*0.25))+exp(-phi*0.25)*exp(-gamma*0.25))*exp(-gamma*(t-0.75)) - 
          gamma*LTN - kappa*lag0.75[1]/(lag0.75[1]+rho*lag0.75[2])*pulsebeta(P0,alpha,beta,1.75-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.75)))+exp(-phi*(t-0.75))*exp(-gamma*(t-0.75)))*exp(-0.25*gamma)
        
        dLTRdt = kappa*rho*lag0.5[2]/(lag0.5[1]+rho*lag0.5[2])*pulsebeta(P0,alpha,beta,1.5-t)*((phi/(phi+gamma))*(1-exp(-phi*0.25))+exp(-phi*0.25)*exp(-gamma*0.25))*exp(-gamma*(t-0.75)) - 
          gamma*LTR - kappa*rho*lag0.75[2]/(lag0.75[1]+rho*lag0.75[2])*pulsebeta(P0,alpha,beta,1.75-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.75)))+exp(-phi*(t-0.75))*exp(-gamma*(t-0.75)))*exp(-0.25*gamma)
        
        dLSNdt = kappa*lag0.75[1]/(lag0.75[1]+rho*lag0.75[2])*pulsebeta(P0,alpha,beta,1.75-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.75)))+exp(-phi*(t-0.75))*exp(-gamma*(t-0.75)))*exp(-0.25*gamma) - phi*LSN - gamma*LSN
        
        dLSRdt = kappa*rho*lag0.75[2]/(lag0.75[1]+rho*lag0.75[2])*pulsebeta(P0,alpha,beta,1.75-t)*((phi/(phi+gamma))*(1-exp(-phi*(t-0.75)))+exp(-phi*(t-0.75))*exp(-gamma*(t-0.75)))*exp(-0.25*gamma) - phi*LSR - gamma*LSR
    ...
    tmp = try({
      ...
      parametersA = unname(unlist((c(mu,P0,gamma,kappa.eff,phi,alpha,beta,rho,P0H))))
      
      ###Functions
      pulsebeta = function(initialI, alpha, beta, time){
        res = rep(NA, length(time))
        for (num in 1:length(time)){
          res[num] = initialI*(dbeta(time[num], alpha, beta))
        }
        return(res)
      }
      ...
      if (bias == "full"){
        ...
        out.A = out.A %>% mutate(.,Ring.adj = Ring - epsilon*(RR+frac*PR)) %>% mutate(.,Late.adj = Late + epsilon*(RR+frac*PR))
  ...
  data.A = DDE.opt(paraA,bias.select,frac)
  data.N = DDE.opt(paraN,bias.select,frac)
  ...
  optim.best = optim.df %>% filter(.,V9==0) %>% top_n(-1,V8) %>% mutate(.,model=model.select) %>% mutate(.,epsilon=epsilon) %>% mutate(.,frac=frac) %>% mutate(.,index=i)
  names(optim.best) = c("lambdaA","kN","kA","c","mu","sigma","P0","fit","con","model","epsilon","frac","index")
  optim.joint = bind_rows(optim.joint,optim.best)
  
  write.csv(optim.joint,paste("Sensitivity_epsilon_",epsilon.frac.opt[set,1],"_frac_",epsilon.frac.opt[set,2],".",variable,".",model.select,".csv",sep=""),row.names=FALSE)
}
